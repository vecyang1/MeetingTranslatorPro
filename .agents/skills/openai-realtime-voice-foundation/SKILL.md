...
- In M7/M8 translation mode, `gpt-realtime-translate` is the primary audio consumer. Forward source-caption sidecar audio to `gpt-realtime-whisper` only after Translate accepts the same chunk; otherwise the UI can create a source-only row that waits forever for missing Translate output. Official Realtime Translation docs say to keep appending audio, including silence between phrases, so a bounded near-silence continuity tail should still go to Translate only after accepted voiced chunks. Do not send unlimited quiet audio. On Stop, clear pending realtime translation indicators after draft finalization because no later Translate event can arrive after disconnect.
