...
- Run `tools/realtime-foundation/run_realtime_mission_verification.sh` without `--local-only` only when a valid `OPENAI_API_KEY` is available, because it sends generated synthetic audio to OpenAI.
