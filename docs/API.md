...
For M7, `gpt-realtime-translate` is the same-time interpretation model. Provider probes showed Translate output transcript/audio events while source transcript events were not consistently emitted from the translation stream itself, so the app fans accepted audio to a paired `gpt-realtime-whisper` source-caption sidecar when the explicit interpreter gate is on. The sidecar is downstream of the Translate accept path: if Translate does not accept a chunk, the sidecar must not create a source-only row that can never receive live translation. A bounded low-energy continuity tail in translation mode is forwarded only to `gpt-realtime-translate` after accepted voiced chunks so the dedicated translation session keeps receiving phrase-boundary context, including silence between phrases, without committing silence to Whisper or sending unlimited quiet audio. Stop finalization clears pending realtime translation state after drafts are promoted so rows cannot remain on `Waiting for live translation...` / `Translating...` after the translation session is gone. The reducer attaches translation output to the latest source-caption item for the same audio source, including the output-first case where Translate text arrives before Whisper source text, and removes the superseded translation-only draft row once it rebinds. This preserves source captions, proves source deltas with the realtime transcription model, and makes cost accounting explicit: M7 with source captions is Translate + Whisper. Whisper must not be described as the translation model.
...
- ScreenCaptureKit exclusion does not stop laptop-speaker bleed into the microphone. When microphone capture is active, `AudioOutputRouteInspector` reads the CoreAudio default output route name, UID, transport, manufacturer, and data source. `RealtimeTranslatedAudioOutputSafety` blocks likely speakers, display audio, HDMI/DisplayPort, AirPlay, aggregate/multi-output routes, and unrecognized outputs even if old playback opt-in or safe-output confirmation was saved. A safety downgrade clears saved playback and stale safe-output confirmation. Confirmation is bound to the current output-route fingerprint and may unlock only positive headphone-like routes; the UI never claims room-speaker safety. `AudioOutputRouteObserver` watches `kAudioHardwarePropertyDefaultOutputDevice` so an output-route change refreshes safety and reroutes/clears active translated playback instead of waiting for another settings action.
...
Before processing, each audio chunk is checked for sufficient energy. This is exposed in Settings as `Audio Input Filter`, not as an engine-specific model control, because it gates microphone/system PCM before Realtime captions, legacy OpenAI, or Gemini routes receive audio. The exception is `gpt-realtime-translate` interpreter continuity: after accepted voiced chunks, a bounded near-silence tail is appended to the Translate session only, because Realtime Translation is continuous and uses silence between phrases as part of the live audio stream:
